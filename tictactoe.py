...
def ask_input(board, name, marker):
    ...
    while position not in [1, 2, 3, 4, 5, 6, 7, 8, 9] or not check_whitespace(board, position):
        try:
            ...
            print("Invalid position, try again please")
    return position
    ...
    while True:
        ...
        while True:
            if next_player == 'Player 1':
                ...
                place_marker(board, available, marker_p1,
                             ask_input(board, next_player, marker_p1))
                print('\n' * 20)  # prints 20 lines to separate the outputs.
                if check_win(board, marker_p1):
                    print("The {} ({}) won. Congrats! ".format(
                        next_player, marker_p1))
                    display_board(available, board)
                    break
                ...
                display_board(available, board)
                place_marker(board, available, marker_p2,
                             ask_input(board, next_player, marker_p2))
                print('\n' * 20)  # prints 20 lines to separate the outputs.
                if check_win(board, marker_p2):
                    print("The {} ({}) won. Congrats! ".format(
                        next_player, marker_p2))
                    display_board(available, board)
                    break
                elif check_full(board):
                    print('The game is a draw!!')
